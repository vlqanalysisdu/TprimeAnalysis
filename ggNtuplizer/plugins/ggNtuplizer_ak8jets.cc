...
vector<float> AK8JetCHSPhi_;
vector<float> AK8JetCHSMass_;
...
void ggNtuplizer::branchesAK8Jets(TTree* tree) {
  ...
  tree->Branch("AK8Jet_tau4",              &AK8Jet_tau4_);
  ...
  tree->Branch("AK8JetCHSPhi",         &AK8JetCHSPhi_);
  tree->Branch("AK8JetCHSMass",         &AK8JetCHSMass_);
  ...
  tree->Branch("AK8puppiTau4",             &AK8puppiTau4_);
...
void ggNtuplizer::fillAK8Jets(const edm::Event& e, const edm::EventSetup& es) {
  ...
  //if(doGenParticles_)e.getByToken(genJetCollection_, genJetHandle);
  edm::Handle<vector<reco::GenParticle> > genJetHandle;
  if(doGenParticles_)e.getByToken(genParticlesCollection_, genJetHandle);
  ...
  edm::Handle<edm::View<pat::Jet> > jetsAK8Sub;
  e.getByToken(jetsAK8SubLabel_, jetsAK8Sub);
  
  if (!jetsAK8Sub.isValid()) {
    edm::LogWarning("ggNtuplizer") << "no pat::Jets (AK8 subjets) in event";
    return;
  }
  ...
  edm::View<pat::Jet>::const_iterator beginAK8Sub = jetsAK8Sub->begin();
  edm::View<pat::Jet>::const_iterator endAK8Sub = jetsAK8Sub->end();
  edm::View<pat::Jet>::const_iterator ijetAK8Sub = beginAK8Sub;
  ...
  for (ijetAK8 = beginAK8; ijetAK8 != endAK8; ++ijetAK8 ) {
    ...
    AK8Jet_tau4_.push_back( ijetAK8->userFloat("ak8PFJetsCHSValueMap:NjettinessAK8CHSTau4") );
 ...
 //    AK8JetPFTightLepVetoId_.push_back(AK8jetIDTightLepVeto);
    ...
    //parton id    
    //cout<<"I am outside before gen  ak8jets "<<endl;
    ...
    float AK8JetGenPhi = -999.;
      ...
      //cout<<"I am inside  gen  ak8jets  just in "<<endl;

    if (doGenParticles_ && genJetHandle.isValid() ) {

      //cout<<"I am inside gen if gen true for  ak8jets "<<endl;

      if ((*ijetAK8).genParton()) 
      {
	AK8JetGenPartonID = (*ijetAK8).genParton()->pdgId();

 // cout<<"pdgID"<<AK8JetGenPartonID<<endl;
  ...
  //Mother Information missing in 94X for ak8GenParticles Dated 28 May 2019
 ...
 //    }
  ...
  //cout<<"ak8jgenjet energy   = "<<AK8JetGenJetEn<<endl;
 ...
 //    AK8SDSJCSV_.push_back(vecSDSJcsv);
    ...
    AK8puppiTau1_.push_back( ijetAK8->userFloat("NjettinessAK8Puppi:tau1"));
    AK8puppiTau2_.push_back( ijetAK8->userFloat("NjettinessAK8Puppi:tau2"));
    AK8puppiTau3_.push_back( ijetAK8->userFloat("NjettinessAK8Puppi:tau3"));
    AK8puppiTau4_.push_back( ijetAK8->userFloat("NjettinessAK8Puppi:tau4"));
  ...
  for (ijetAK8Sub = beginAK8Sub; ijetAK8Sub != endAK8Sub; ++ijetAK8Sub ) {
    ...
    //for ( auto const & puppiSDSJ : sdSubjetsPuppi ) {
      ...
      vecPuppiSDSJpt.push_back(ijetAK8Sub->pt());
      vecPuppiSDSJeta.push_back(ijetAK8Sub->eta());
      vecPuppiSDSJmass.push_back(ijetAK8Sub->mass());
      vecPuppiSDSJphi.push_back(ijetAK8Sub->phi());
      vecPuppiSDSJe.push_back(ijetAK8Sub->energy());
      vecPuppiSDSJflavour.push_back(abs(ijetAK8Sub->partonFlavour()));
      vecPuppiSDSJcharge.push_back(ijetAK8Sub->charge());
      vecPuppiSDSJcsv.push_back(ijetAK8Sub->bDiscriminator("pfCombinedInclusiveSecondaryVertexV2BJetTags") );
      
      puppi_softdrop_subjet.SetPtEtaPhiM(ijetAK8Sub->correctedP4(0).pt(),ijetAK8Sub->correctedP4(0).eta(),ijetAK8Sub->correctedP4(0).phi(),ijetAK8Sub->correctedP4(0).mass());
